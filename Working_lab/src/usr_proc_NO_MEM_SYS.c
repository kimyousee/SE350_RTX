/**
 * @file:   usr_procA.c
 * @brief:  Two user processes: proc1...2 to do request empty memory and call system processes; should be blocked
 * @author: G031
 * @date:   2016/02/03
 ...
 * requests all memory blocks and blocks itself
 */
void proc1(void)
{
	int i = 0;
	...
	while ( 1 ) {
		...
		if ( i != 0 && i==10) {
			uart0_put_string("\n\rproc1 requesting for all memory\n\r");
...
/**
 * @brief: process creates message when i = 5, and sends message to KCD at i = 15
 */
void proc2(void)
{
	int i = 0;
