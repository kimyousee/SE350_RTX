...
void proc1(void)
{
	...
	while ( 1 ) {
		...
		if ( i != 0 && i==5) {
			...
			p_msg_env->mtext[8] = '2';
			p_msg_env->mtext[9] = ':';
			p_msg_env->mtext[10] = '1';
			p_msg_env->mtext[11] = '2';
	...
	uart0_put_string((unsigned char *)c);
	while ( 1 ) {
		release_processor();
	}
}
...
void proc2(void)
{
	...
	while ( 1) {
		...
		if ( i!=0 && i==30 ) {
			...
			p_msg_env->mtext[5] = '1';
			p_msg_env->mtext[6] = '\0';
			send_message(PID_SET_PRIO, (void *)p_msg_env);
		}
